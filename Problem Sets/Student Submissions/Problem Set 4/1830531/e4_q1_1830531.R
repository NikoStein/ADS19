...
bb <- read_csv2("basketball_complete.csv")
...
bb %>%
  mutate(season = as.numeric(str_sub(season, 1, 4))) -> bb

bb %>%
  mutate(seconds = seconds_remaining + 60*minutes_remaining) %>%
  mutate(lastsecond = ifelse(seconds < 2, 1, 0)) -> bbmore
bb_split <- initial_split(bbmore, prop = 0.80, strata = "shot_made_flag")
bb_train <- training(bb_split)
bb_test <- testing(bb_split)

bb_recipe <- recipe(shot_made_flag ~ ., data = bb_train)
...
bb_recipe %>%
  ...
  step_scale(all_predictors()) -> bb_recipe_steps
...
glmn <- logistic_reg(penalty = 0.005, mixture = 0.5) %>%
  set_engine("glmnet") %>%
  fit(shot_made_flag ~ ., data = bb_train_preprocessed)
glmn
